...
<? print_top($prog_name); ?>
<? print_navigation('Storico','Home Page','../../contents.php','Magazzino','../magazzino_index.php'); ?>
<? print_title('Storico'); ?>
...
<?
    ...
    // leggo gli articoli
    $query="SELECT count(*) FROM movimenti";
    ...
    if ($num_rows=='0') {
        ...
        // stampo il risultato
        $query="SELECT oid,* FROM movimenti ORDER BY data_movimento";
        $result = db_execute($conn,$query);
        if ($DEBUG) { print 'Query: <b>' . $query . '</b><br>'; };
        if (!$result) {
            if ($DEBUG) { print 'file history error: cannot execute query.\n'; };
            exit;
        };
        ...
        print '    <td width="55%"><font face="arial,helvetica,sans-serif" size="2" style="color: white">Movimento</font></td>';
